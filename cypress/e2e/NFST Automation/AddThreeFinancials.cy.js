...
describe('Testing of the SPreading Tool', function() {
 ...
 beforeEach(() => {
  // Perform login before each `it` block
  cy.visit('https://qa.st.xgencredit.com');
  cy.get('.mui-1tb6yvl').contains("Welcome Back");
  cy.get('#username').type('pasal');
  cy.get('#password').type('st@123');
 
// Check if the button is visible, click if visible, else continue
...
.then($button => {
  if ($button.is(':visible')) {
    ...
    // If the button is not visible, continue with the rest of the code
...
// verifyKPIValues('#profitability_totalRevenue_0', TotalRevenue, 'Total Revenue');

// cy.log('The EBITDAKPI is'+ EBITDAKPI );
// // Verification of Sales and Rental Income
// verifyKPIValues('#profitability_ebitda_0', EBITDAKPI, 'EBITDA');

//   // cy.get('#profitability_totalRevenue_0')
//   //   .invoke('text') // Get the text content of the element
//   //   .then((text) => {
   
//   //     expect(actualValue).to.eq(TotalRevenue); // Assert that they are equal
//   //     cy.log(`The Actual value is ${actualValue}. The expected value is ${TotalRevenue}. (${description})`);
//   //   });

//   //   cy.get('#profitability_ebitda_0')
//   //   .invoke('text') // Get the text content of the element
//   //   .then((text) => {
//   //     const actualValue = text // Convert the text to a number
//   //     expect(actualValue).to.eq(EBITDAKPI); // Assert that they are equal
//   //     cy.log(`The Actual value is ${actualValue}. The expected value is ${EBITDAKPI}. (${description})`);
//   //   });
